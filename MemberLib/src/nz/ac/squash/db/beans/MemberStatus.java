...
public class MemberStatus {
	...
	public static Collection<MemberStatus> getPresentMembers() {
		// TODO Avoid copy.
		final List<MemberStatus> toReturn = new ArrayList<MemberStatus>();
		...
		DB.executeTransaction(new Transaction() {
			...
			public void run() {
				...
				toReturn.addAll(typedQuery(
						MemberStatus.class,
						"select s from "
								+ MemberStatus.class.getName()
								+ " as s where s.mDate >= ?0 and s.mDate = (select max(mDate) from "
								+ MemberStatus.class.getName()
								+ " as m where s.mMember = m.mMember)", today));
			}
		});
