...
public class RegisterWindow extends JDialog {
	...
	private void doRegistration() {
		...
		if (mSkill1Radio.isSelected())
			proto.setSkillLevel("1");
		else if (mSkill2Radio.isSelected())
			proto.setSkillLevel("2");
		else if (mSkill3Radio.isSelected())
			proto.setSkillLevel("3");
		else if (mSkill4Radio.isSelected())
			proto.setSkillLevel("4");

		if (mStudentYesRadio.isSelected())
			proto.setStudentStatus("Yes");
		else if (mStudentAlumniRadio.isSelected())
			proto.setStudentStatus("Alumni");
		else if (mStudentNoRadio.isSelected())
			proto.setStudentStatus("No");

		proto.setStudentId(mStudentIdField.getText());
		proto.setUPI(mUpiField.getText());

		if (mPaymentCashRadio.isSelected())
			proto.setPaymentMethod("Cash at club night");
		else if (mPaymentTransferRadio.isSelected())
			proto.setPaymentMethod("Bank transfer");
		...
		// Export to CSV.
		if (mExportFile == null) {
			mExportFile = new File("logs/Submissions from "
					+ Utility.FILE_SAFE_FORMATTER.format(new Date()) + ".csv");
		}
