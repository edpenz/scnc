...
public class JBrandedPanel extends JPanel {
	...
	private static final Color BASE_COLOR = Color.decode("#0153A5");
	private static final Image BASE_IMAGE;
	private static final Color FADE_COLOR = new Color(
			BASE_COLOR.getRed() / 255.0f, BASE_COLOR.getGreen() / 255.0f,
			BASE_COLOR.getBlue() / 255.0f, 0.75f);
	static {
		Image loaded = null;
		try {
			loaded = ImageIO.read(ClassLoader
					.getSystemResource("images/racquet_white.png"));
		} catch (Exception e) {
